...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  cookies: academiaCookies(process.env.AUTH_URL?.startsWith("https://") ?? process.env.NODE_ENV === "production"),
  providers: [Credentials({
    ...
    async authorize(input) {
      try {
        return await authenticateCredentials(input, createUsersRepository(getDb()), getLoginLimiter());
      } catch (error) {
  ...
  callbacks: {
    async jwt({ token, user }) {
      ...
      if (!id) return null;
      try {
        const identity = await currentIdentity(id, createUsersRepository(getDb()));
