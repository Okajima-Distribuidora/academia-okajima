...
export class LocalLoginLimiter {
  ...
  constructor(private readonly now = Date.now, private readonly maxEntries = 10_000) {}

  private consume(scope: string, value: string, limit: number): () => void {
    const now = this.now();
    for (const [key, entry] of this.buckets) if (entry.until <= now) this.buckets.delete(key);
    const key = createHmac("sha256", this.key).update(JSON.stringify([scope, value])).digest("hex");
  ...
  consumeOrigin(origin: string) { return this.consume("origin", origin, 30); }
  consumeIdentifier(origin: string, identifier: string) {
    return this.consume("identifier", JSON.stringify([origin, identifier.toLowerCase()]), 5);
  }
...
const shared = globalThis as typeof globalThis & { academiaFailedLoginLimiterV2?: LocalLoginLimiter };
